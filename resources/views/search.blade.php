    ...
    <div class="container mx-auto flex justify-center bg-gray-100" style="min-height:90vh">
        <div class="w-3/5 bg-white rounded-lg p-8 shadow-md my-4" style="width:60%">
            ...
            <div class="flex justify-center gap-2 mb-8 w-full max-w-md mx-auto">
                <button
                    class="cursor-pointer hover:bg-blue-600 filter-btn flex-1 px-2 py-1 rounded bg-blue-500 text-white text-sm focus:outline-none"
                    data-target="posts">Posts</button>
                <button
                    class="cursor-pointer hover:bg-blue-600 filter-btn flex-1 px-2 py-1 rounded bg-blue-500 text-white text-sm focus:outline-none"
                    data-target="users">Users</button>
                <button
                    class="cursor-pointer hover:bg-blue-600 filter-btn flex-1 px-2 py-1 rounded bg-blue-500 text-white text-sm focus:outline-none"
                    data-target="categories">Categories</button>
            </div>
            ...
            <div id="posts" class="filter-section mb-8">
                ...
                @forelse($posts as $post)
                    @php
                        $img = $post->additionFile
                            ? asset('storage/uploads/' . $post->additionFile)
                            : asset('images/free-images-for-blog.png');
                    ...
                    @endphp
                    <div
                        class="bg-white rounded-lg shadow p-4 flex flex-row gap-4 items-center mb-4 hover:shadow-lg transition min-h-[120px]">
                        <img src="{{ $img }}" alt="Post Image"
                            class="w-20 h-20 object-cover rounded-md bg-gray-100"
                            onerror="this.src='{{ asset('images/free-images-for-blog.png') }}'">
                        <div class="flex-1 min-w-0">
                            <a href="{{ route('post.content.viewer', ['id' => $post->id]) }}"
            ...
            <div id="users" class="filter-section mb-8 hidden">
                ...
                @forelse($users as $user)
                    ...
                    @endphp
                    <div
                        class="flex items-center gap-4 mb-4 bg-white rounded-lg shadow p-4 hover:bg-gray-100 transition">
                        <img src="{{ $avatar }}" alt="Avatar"
                            class="w-12 h-12 object-cover rounded-full bg-white"
                        ...
                        <div class="flex flex-col">
                            <a href="/user-profile/{{ $user->id }}"
                                class="font-bold text-base text-black cursor-pointer user-title-hover hover:text-blue-600 hover:underline-0"
