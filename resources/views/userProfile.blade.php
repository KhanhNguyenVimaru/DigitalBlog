...
<body class="bg-gray-100" style = "min-height: 80vh">
    @include('header')
            ...
            <div
                ...
                <div class="flex flex-col items-start justify-center flex-1 pl-4">
                    <div class="flex flex-row items-center gap-2 mb-1">
                        ...
                        @if(Auth::id() !== $user->id)
                            @if($already_followed)
                                <button id="unfollow-btn" class="bg-gray-400 cursor-pointer text-white font-semibold px-4 py-1 rounded-full shadow transition text-xs">Following</button>
                            @elseif($request_sent)
                                <button id="revoke-request-btn" class="bg-gray-400 cursor-pointer text-white font-semibold px-4 py-1 rounded-full shadow transition text-xs">Pending</button>
                            @elseif($can_request_again)
                                <button id="request-btn" class="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-4 py-1 rounded-full shadow transition text-xs">Request</button>
                            @else
                                <button id="follow-btn" class="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-4 py-1 rounded-full shadow transition text-xs">Follow</button>
                            @endif
                        @else
                    ...
                    @elseif(isset($followers))
                        @foreach($followers as $item)
                            ...
                            <a href="{{ route('userProfile', ['id' => $f->id]) }}" class="flex items-center gap-4 p-4 hover:bg-gray-50 transition rounded cursor-pointer">
                                <img src="{{ $f->avatar ?? 'https://static.vecteezy.com/system/resources/previews/009/292/244/non_2x/default-avatar-icon-of-social-media-user-vector.jpg' }}" class="w-14 h-14 rounded-full" alt="avatar">
                    ...
                    posts.forEach(post => {
                        ...
                        postDiv.innerHTML = `
                            ...
                            <div class="flex-1 min-w-0">
                                <a href="/post-content-viewer/${post.id}" class="font-bold text-base text-black cursor-pointer post-title-hover hover:text-blue-600 hover:underline-0 line-clamp-2" style="text-decoration: none;">${post.title}</a>
                                <div class="text-gray-600 text-sm mt-1 line-clamp-2">${post.preview || ''}</div>
                                <div class="flex flex-row items-center gap-2 mt-2">
                                    <span class="inline-block truncate px-2 py-1 rounded-full bg-blue-100 text-blue-700 text-xs font-semibold cursor-pointer" style="max-width:100px">${categoryName}</span>
