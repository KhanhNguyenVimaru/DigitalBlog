...
<body>
                ...
                @foreach ($topLikedPosts as $post)
                    <div class="bg-white p-4 rounded shadow hover:shadow-lg transition-shadow duration-300">
                        ...
                        <div class="flex flex-row items-center gap-2 mt-2">
                            <a class="text-sm text-gray-700 font-medium hover:text-blue-600 cursor-pointer"
                                href="{{ url('/user-profile/' . $post->authorId) }}">
                                {{ $post->author->name }}
                            </a>
                            <span
                                class="ml-auto inline-block truncate px-2 py-1 rounded-full bg-blue-100 text-blue-700 text-xs font-semibold cursor-pointer">{{ $post->category->content ?? 'No Category' }}</span>
                        </div>
                    </div>
                @endforeach
            </div>
    ...
    <div class="w-full mt-4 min-h-[100vh]">
        <div class="max-w-screen-xl mx-auto h-full pb-5">
            <!-- Navigation -->
            <div class="pl-4 flex justify-start mb-4 gap-8 text-center" id="nav-links">
                <a data-filter="latest"
                    class="filter-link text-base text-black px-4 cursor-pointer" aria-current="true">LATEST</a>
                <a data-filter="popular"
                    class="filter-link text-base text-gray-500 hover:text-black px-4 cursor-pointer" aria-current="false">MOST POPULAR</a>
                <a data-filter="interaction"
                    class="filter-link text-base text-gray-500 hover:text-black px-4 cursor-pointer" aria-current="false">TOP INTERACTION</a>
            </div>
            ...
            <div class="grid grid-cols-12 gap-6 h-full mb-10">
                ...
                <div class="col-span-3 rounded-lg">
                    <div class="w-full p-4 bg-white shadow-md rounded-xl mt-2">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function loadPosts(sortBy = 'latest', page = 1) {
                ...
                fetch(`/api/posts/${sortBy}?show-page=${page}`, {
                    headers: {
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            ...
                            if (data.posts.data.length > 0) {
                                ...
                                // Render pagination
                                postsContainer.insertAdjacentHTML('beforeend', renderPagination(data.posts, sortBy));
                            } else {
            ...
            document.addEventListener('click', function (e) {
                if (e.target.classList.contains('page-link')) {
                    ...
                    const activeFilter = document.querySelector('.filter-link.text-black')?.getAttribute('data-filter') || 'latest';
